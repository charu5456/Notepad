...
class UI {
    ...
    static addNoteToList(note) {
        ...
        card.classList = 'card text-white bg-warning mb-2';

        card.innerHTML = `
            <div class="card-header">
                <span>${note.title}</span>
                <i class="fas fa-trash" noteid="${note.id}"></i>
                <i class="fas fa-pencil-alt mr-5" noteid="${note.id}"></i>
            </div>
            <div class="card-body ${note.color} ${note.color=='bg-white'?'text-danger':''}">
